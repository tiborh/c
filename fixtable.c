...
int main(int argc, char *argv[])
{
  ...
  short int table_flag=0,limbo_flag=0,list_limbo=0,row_flag=0,caption_flag=0,steplist_flag=0,sli_flag=0,title_flag=0,li_flag=0,entry_flag=0,tp_flag=0,list_change_flag=0,p_flag=0,list_protect_flag=0,input_flag=0,p_change_flag=0,emph_flag=0;
  short int row_counter=0,cell_counter=0,li_depth=0,list_dropped=0,emph_counter=0;
  ...
  while(fgets(in_line,sizeof(in_line)-1,in_file) != NULL)
    {
      ...
      if(strstr(in_line,"<emph") != NULL)
	if (strstr(in_line,"</emph>") == NULL)
	  {
	    emph_counter++;
	    printf("emph_counter: %d\n",emph_counter);
	  }
      if(strstr(in_line,"</emph>") != NULL)
	if (strstr(in_line,"<emph") == NULL)
	  {
      ...
      if(strstr(in_line,"</p>")!=NULL && (emph_counter > 0))
	{
	  printf("</emph>");
	  emph_counter--;
      ...
      for (i=1,j=MAX_HEAD_DEPTH;i<=MAX_HEAD_DEPTH;i++,j--)
	{
      ...
      /* checking input tag */
      if((input_flag == 1) && ((strstr(in_line,"</p>") != NULL) || (strstr(in_line,"<input>") != NULL) || (strstr(in_line,"<p>") != NULL)))
	{
	  printf("</input>\n");
	  input_flag=0;
	  bprint("input flag",input_flag);
	}
      if (strstr(in_line,"<input>") != NULL)
	{
	  input_flag=1;
	  bprint("input flag",input_flag);
	}
      ...
      if((p_flag == 1) && (strstr(in_line,"<step-list>") != NULL))
	{
      ...
      if((p_flag == 1) && (strstr(in_line,"<list>") != NULL))
	{
	  printf("</p>\n");
	  p_flag=0;
	}
      ...
      if(strstr(in_line,"</p>") != NULL)
	{
      ...
      if((p_flag == 1) && (strstr(in_line,"<table>") != NULL))
	  printf("</p>\n");
      if((p_flag == 1) && (strstr(in_line,"</table>") != NULL))
	strcat(in_line,"<p>\n");

      if(strstr(in_line,"xlink:href") != NULL)
	{
	  if(p_flag==1)
	    {
	      printf("</p>\n");
	  ...
	  printf("<figure>\n");
	  printf("<graphics xlink:type=\"simple\"\n");
	  printf("%s",in_line);
	  printf("xlink:show=\"embed\" xlink:actuate=\"onLoad\"\n");
      ...
      if (get_substr(in_line,"/row") == 0)
	{
      ...
      if((strstr(in_line,"<entry") != NULL) && (entry_flag == 1))
	  continue;
