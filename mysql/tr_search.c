...
char* get_query(void)
{
  ...
  char *data;
  char *query_txt = malloc(sizeof(char) * QUERYLENGTH + INPUTSIZE + 1);
...
from TR left join TRANSWER on TRANSWER.eriref=TR.eriref\n\
  left join TRCLONEFROM on TR.eriref=TRCLONEFROM.eriref\n\
