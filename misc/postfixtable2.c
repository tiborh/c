...
int main(int argc, char **argv)
{
  ...
  char *infile_name = argv[1]; /* the first argument is the name of the input
				  file */
  char cmd_nam_str[LINEWIDTH+1],in_line[LINEWIDTH+1],old_line[LINEWIDTH+1],str_extract[LINEWIDTH/4],str_assembled[LINEWIDTH/4];
  ...
  while(fgets(in_line,sizeof(in_line)-1,in_file) != NULL)
    {
      if((strstr(old_line,"<title>") != NULL) && (strstr(in_line,"<xref") != NULL))
	{
	  get_strfromtag(in_line,"xref",str_extract);
	  strcpy(str_assembled,"<title xml:id=\"\0");
	  strcat(str_assembled,str_extract);
	  strcat(str_assembled,"\">");
	  strepl(old_line,"<title>",str_assembled);
	  tag_del(in_line,"xref");
	}
      printf("%s",old_line);
