...
int main(int argc, char **argv)
{
  ...
  char *infile_name = argv[1]; /* the first argument is the name of the input
				  file */
  int hold_flag=0,list_hold=0,sl_hold=0,slhold_flag=0;
  ...
  while(fgets(in_line,sizeof(in_line)-1,in_file) != NULL)
	...
	strepl(in_line,"<!--</list>-->","");
     ...
     /* to place embedded <list> environment into the <list-item> before */
      if((strstr(old_line,"</p></list-item>") != NULL) && (strstr(in_line,"<list>") != NULL))
	{
	  strepl(old_line,"</p></list-item>","</p>");
	  strcpy(hold_line,"</list-item>");
	  hold_flag=1;
	}

      if((strstr(in_line,"</list>") != NULL) && (hold_flag==1))
	{
	  if(list_hold==0)
	    list_hold=1;
	  else
